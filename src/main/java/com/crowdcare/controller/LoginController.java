...
public class LoginController {
    ...
    /*
     * Slide berganti otomatis setiap 7 detik.
     * Angka 7 bisa diganti kalau ingin lebih lambat.
     */
    ...
    private void showSlide(int index, boolean animated) {
        ...
        fadeOut.setOnFinished(event -> {
            oldSlide.setVisible(false);
            oldSlide.setManaged(false);

            newSlide.setVisible(true);
            newSlide.setManaged(true);
            newSlide.setOpacity(0.0);
            ...
            fadeIn.setOnFinished(finishedEvent ->
                    isAnimating = false
            );
    ...
    private void updateIndicators() {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i].getStyleClass().removeAll(
                    "story-dot",
                    "story-dot-active"
            );
    ...
    private void handleForgotPassword() {
        showAlert(
                ...
                "Lupa Kata Sandi",
                "Fitur pemulihan kata sandi masih dalam pengembangan."
        );
    }
    ...
    private void handleRegister(ActionEvent event) {
        try {
            URL registerUrl = getClass().getResource(
                    "/view/register.fxml"
            );

            if (registerUrl == null) {
                throw new IOException(
                        "File register.fxml tidak ditemukan."
                );
            }
